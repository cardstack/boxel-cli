...
describe('pull integration', () => {
  ...
  it('pulls .realm.json via HEAD check', async () => {
    realmState = createRealmState({
      '.realm.json': { content: '{"name":"Test Realm"}' },
    ...
    expect(fs.existsSync(path.join(localDir, '.realm.json'))).toBe(true);
    expect(fs.readFileSync(path.join(localDir, '.realm.json'), 'utf-8'))
      .toBe('{"name":"Test Realm"}');

    // Verify HEAD request was made
    const headCalls = calls.filter(c => c.method === 'HEAD');
    expect(headCalls.length).toBeGreaterThanOrEqual(1);
    expect(headCalls.some(c => c.url.includes('.realm.json'))).toBe(true);
  });
