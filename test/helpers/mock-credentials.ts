...
/**
 * Creates a valid 3-part base64 JWT with a future `exp` timestamp.
 * RealmAuthClient at realm-auth-client.ts:35 does:
 *   JSON.parse(atob(this._jwt.split('.')[1]))
 * So the payload must be valid base64-encoded JSON with `exp`.
 */
export function createMockJWT(overrides: Partial<{
  exp: number;
  user: string;
  realm: string;
  permissions: string[];
  ...
  // Use btoa-compatible encoding (works in Node 16+)
...
/**
 * Returns a valid OpenID token response as returned by Matrix.
 */
