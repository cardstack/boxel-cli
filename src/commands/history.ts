    ...
    if (typeof options.restore === 'string') {
      ...
      if (!isNaN(num) && num >= 1 && num <= checkpoints.length) {
        ...
        targetCheckpoint = checkpoints.find(cp =>
          cp.hash.startsWith(input) || cp.shortHash === input
        );
      }
...
function formatSource(source: 'local' | 'remote' | 'manual'): string {
  if (source === 'local') return `${FG_GREEN}LOCAL (you pushed)${RESET}`;
  if (source === 'remote') return `${FG_CYAN}SERVER (external change)${RESET}`;
  return `${FG_MAGENTA}MANUAL${RESET}`;
}

async function quickRestore(manager: CheckpointManager, checkpoint: Checkpoint): Promise<void> {
