...
interface TouchOptions extends SyncOptions {
  files?: string[];
}

class RealmToucher extends RealmSyncBase {
  hasError = false;
  private filesToTouch: string[];
  ...
  async touch(): Promise<void> {
    console.log(
      `Touching files in ${this.options.workspaceUrl} to force re-indexing`,
    );
    ...
    try {
      ...
      throw new Error(
        'Cannot proceed: Authentication or access failed. ' +
    ...
    let filesToProcess: Map<string, string>;
  ...
  private touchJson(content: string): string {
    try {
  ...
  private touchGts(content: string): string {
