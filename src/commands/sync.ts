...
interface SyncManifestFile {
  localHash: string;      // Hash of file content at last sync
...
// Old push-only manifest format (for migration)
interface OldManifest {
  workspaceUrl: string;
  files: Record<string, string>; // Just hash strings
}

function isOldManifest(manifest: unknown): manifest is OldManifest {
  if (!manifest || typeof manifest !== 'object') return false;
  const m = manifest as Record<string, unknown>;
  if (typeof m.files !== 'object' || !m.files) return false;
  // Check if files values are strings (old format) vs objects (new format)
  const firstValue = Object.values(m.files as Record<string, unknown>)[0];
  return typeof firstValue === 'string';
}

type ConflictStrategy = 'manual' | 'local' | 'remote' | 'newest';

interface SyncCommandOptions extends SyncOptions {
  preferLocal?: boolean;
...
interface FileAction {
  type: 'push' | 'pull' | 'conflict' | 'delete-local' | 'delete-remote' | 'none';
  relativePath: string;
  localPath?: string;
  reason: string;
  localMtime?: number;
...
function loadManifest(localDir: string, remoteMtimes?: Map<string, number>): SyncManifest | null {
  ...
  if (fs.existsSync(manifestPath)) {
    try {
      ...
      if (isOldManifest(data)) {
        ...
        const oldManifest = data as OldManifest;
...
class RealmSyncer extends RealmSyncBase {
  ...
  private getConflictStrategy(): ConflictStrategy {
    ...
    if (this.syncOptions.preferNewest) return 'newest';
    return 'manual';
  }

  async sync(): Promise<void> {
    console.log(
      `Starting bidirectional sync: ${this.options.localDir} <-> ${this.options.workspaceUrl}`,
    );
    ...
    if (!this.syncOptions.dryRun) {
      ...
      if (pushedFiles.length > 0) {
        const pushChanges: CheckpointChange[] = pushedFiles.map(f => ({
          file: f,
          status: 'modified' as const,
        }));
  ...
  private computeActions(
    ...
    remoteFiles: Map<string, boolean>,
  ...
  private async resolveConflict(conflict: FileAction): Promise<'push' | 'pull' | 'skip' | 'delete-remote'> {
