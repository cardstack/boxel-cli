...
export class CheckpointManager {
   ...
   */
  getCheckpoints(limit: number = 50): Checkpoint[] {
    if (!this.isInitialized()) {
      return [];
    }
    ...
    return log.trim().split('\n').map(line => {
      ...
      const source = subject.includes('[local]') ? 'local' as const :
                     subject.includes('[remote]') ? 'remote' as const : 'manual' as const;
   ...
   */
  private getCommitStats(hash: string): { filesChanged: number; insertions: number; deletions: number } {
    try {
