...
function cleanseString(value: string): string {
...
export function iconURLFor(word: string): string | undefined {
  ...
  const cleansedWord = cleanseString(word).replace(/^[0-9]+/, '');
  return ICON_URLS[cleansedWord.charAt(0)];
}

export function getRandomBackgroundURL(): string {
  const index = Math.floor(Math.random() * BACKGROUND_URLS.length);
  return BACKGROUND_URLS[index];
}

const ICON_URLS: Record<string, string> = Object.freeze({
